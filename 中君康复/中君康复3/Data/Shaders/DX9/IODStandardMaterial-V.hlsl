...
Output vMain(Input In)
{
    Output Out;
	// Function call #0
    float4 WorldPos_CallOut0, ViewPos;
    TransformPosition(In.Position, g_World, WorldPos_CallOut0);

	// Function call #1
	
	ProjectPositionWorldToProj(WorldPos_CallOut0, g_View, ViewPos);
    ProjectPositionWorldToProj(ViewPos, g_Proj, Out.PosProjected);
	Out.FogOut = saturate( (ViewPos.z - FogStart) / ( FogEnd - FogStart ));
    ...
    NormalizeFloat3(WorldNrm_CallOut4, VectorOut_CallOut5);

	// Function call #6
    //float3 AmbientAccumOut_CallOut6;
    //float3 DiffuseAccumOut_CallOut6;
    //float3 SpecularAccumOut_CallOut6;
    //Light(WorldPos_CallOut0, VectorOut_CallOut5, int(1), bool(false), 
	...
	// Function call #7
    float3 WorldViewVector_CallOut7;
    CalculateViewVector(WorldPos_CallOut0, g_EyePos, WorldViewVector_CallOut7);

	// Function call #8
    float3 VectorOut_CallOut8;
    NormalizeFloat3(WorldViewVector_CallOut7, VectorOut_CallOut8);

	// Function call #9
    float3 WorldReflect_CallOut9;
    WorldReflect(WorldNrm_CallOut4, WorldViewVector_CallOut7, bool(true), 
        WorldReflect_CallOut9);

	// Function call #10
    float3 VectorOut_CallOut10;
    NormalizeFloat3(WorldReflect_CallOut9, VectorOut_CallOut10);
	
	// Function call #12
    ProjectTextureCoordinates(VectorOut_CallOut10, 
        g_EnvironmentMapWorldProjectionTransform0, Out.UVSet3);
		
	Out.DiffuseAccum = float4(1,1,1,1);
	return Out;
}
