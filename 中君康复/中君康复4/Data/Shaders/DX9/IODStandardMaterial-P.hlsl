...
sampler2D glowMap  : register(s5);

bool darkEnable 	= false;
bool decalEnable 	= false;
...
Output pMain(Input In)
{
	...
	// Function call #4			// baseColor lerp decalColor
	float3 BaseDecalColor = BaseColor;
    float4 DecalColor = float4(0,0,0,0);
	if( decalEnable )
	{			
		TextureRGBASample(In.UVSet2, decalMap, bool(false), DecalColor);		
		BaseDecalColor = lerp( BaseColor, DecalColor.xyz, DecalColor.w );
	}
	
	// Function call #5			// glossMap采样
    float3 GlossColor = float3(1,1,1);
	if( glossEnable )
		TextureRGBSample(In.UVSet0, glossMap, bool(false), GlossColor);

	// Function call #6
    float3 EnvColor = float3(0,0,0);
	if( envEnable )				// envMap采样
		TextureRGBSample(In.UVSet3, envMap, bool(false), EnvColor);
		
	float3 GlowColor = float3(0,0,0);
	if( glowEnable )
		TextureRGBSample(In.UVSet0, glowMap, bool(false), GlowColor);	

	// Function call #7			// glossColor * envColor
	float3 GlossEnvColor;
    MultiplyFloat3(GlossColor, EnvColor, GlossEnvColor);
	...
	// Function call #11
    float3 OutputColor;			// + glossEnvColor
    CompositeFinalRGBColor(DiffuseBaseColor, GlossEnvColor, OutputColor);
	CompositeFinalRGBColor(OutputColor, GlowColor, OutputColor);
